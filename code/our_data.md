...
**LPI\_only\_obs:** This is the Living Planet data set in time series
format (each observation is a Year with a Count)

**redlist\_data:** This is the original dataset from IUCN Redlist

**merged\_data:** This is the merged dataset of LPI\_data and
redlist\_data (not time series)
...
**expenditures\_timeseries:** Time series expenditure data
(expenditures\_200X compressed into one set)
...
``` r
#Code for plot:
LPI_red_expen_ts%>%filter(Common_name=="Desert tortoise")%>%group_by(ID,Year)%>%ggplot(aes(Year,Countz))+geom_smooth(alpha=0.3)+geom_smooth(aes(y=Species_total/10000000,col="Expenditure"),alpha=0)+scale_y_continuous(name="Count (Z score)",sec.axis = sec_axis(~.*100, name ="Expenditure ($10,000,000"))+labs(title="Desert Tortoise")
```
...
![](our_data_files/figure-gfm/plots-1.png)<!-- -->

``` r
LPI_red_expen_ts%>%filter(Common_name=="Steller sea lion")%>%group_by(ID,Year)%>%ggplot(aes(Year,Countz))+geom_smooth(alpha=0.3)+geom_smooth(aes(y=Species_total/10000000,col="Expenditure"),alpha=0)+scale_y_continuous(name="Count (Z score)",sec.axis = sec_axis(~.*100, name ="Expenditure ($10,000,000"))+labs(title="Steller Sea Lion")
```
...
![](our_data_files/figure-gfm/plots-2.png)<!-- -->

``` r
LPI_red_expen_ts%>%filter(Common_name=="Bald eagle")%>%group_by(ID,Year)%>%ggplot(aes(Year,Countz))+geom_smooth(alpha=0.3)+geom_smooth(aes(y=Species_total/10000000,col="Expenditure"),alpha=0)+scale_y_continuous(name="Count (Z score)",sec.axis = sec_axis(~.*100, name ="Expenditure ($10,000,000"))+labs(title="Bald Eagle")
```
...
![](our_data_files/figure-gfm/plots-3.png)<!-- -->

``` r
LPI_red_expen_ts%>%filter(Common_name=="Loggerhead sea turtle / Loggerhead turtle")%>%group_by(ID,Year)%>%ggplot(aes(Year,Countz))+geom_smooth(alpha=0.3)+geom_smooth(aes(y=Species_total/10000000,col="Expenditure"),alpha=0)+scale_y_continuous(name="Count (Z score)",sec.axis = sec_axis(~.*100, name ="Expenditure ($10,000,000"))+labs(title="Loggerhead Turtle")
```

    ## `geom_smooth()` using method = 'loess' and formula 'y ~ x'
    ## `geom_smooth()` using method = 'loess' and formula 'y ~ x'

    ## Warning: Removed 441 rows containing non-finite values (stat_smooth).

![](our_data_files/figure-gfm/plots-4.png)<!-- -->

``` r
LPI_red_expen_ts%>%filter(Common_name=="Rainbow trout / Steelhead trout")%>%group_by(ID,Year)%>%ggplot(aes(Year,Countz))+geom_smooth(alpha=0.3)+geom_smooth(aes(y=Species_total/10000000,col="Expenditure"),alpha=0)+scale_y_continuous(name="Count (Z score)",sec.axis = sec_axis(~.*100, name ="Expenditure ($10,000,000"))+labs(title="Rainbow Trout")
```
...
![](our_data_files/figure-gfm/plots-5.png)<!-- -->

``` r
LPI_red_expen_ts%>%filter(Common_name=="Chinook salmon")%>%group_by(ID,Year)%>%ggplot(aes(Year,Countz))+geom_smooth(alpha=0.3)+geom_smooth(aes(y=Species_total/100000000,col="Expenditure"),alpha=0)+scale_y_continuous(name="Count (Z score)",sec.axis = sec_axis(~.*100, name ="Expenditure ($10,000,000"))+labs(title="Chinook Salmon")
```
...
![](our_data_files/figure-gfm/plots-6.png)<!-- -->
